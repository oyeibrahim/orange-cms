...
<body>
	...
	<%
		...
		response.sendRedirect("backend/posts/page/1/");
	...
	//to know if its admin making the changes
	session.setAttribute("adminEditing", "Editor is Admin");
	%>
		...
		<div class="row">
			...
			<div class="col-md-10" style="padding: 20px;" id="main">
				...
				<div class="img-thumbnail bg-grey container"
					...
					<h4 class="text-right">
						<a href="<%=webAddress%>/PostDelete">Delete Post</a>
					</h4>
					...
					<form class="needs-validation" style="padding: 10px;"
						action="../PostTypeStatusEdit" method="post" id="auth-form"
						novalidate>
						...
						<div class="form-label-group">
							<select class="form-control shadow" id="status" name="status">
								<%
									//grab all possible status and set selected to the existing status
								SiteStructureDAO stDAO = new SiteStructureDAO();
								SiteStructure st = stDAO.getStructure("post_status");
								//set the three values to an array for easy iteration
								String[] statuses = {st.getText_value1(), st.getText_value2(), st.getText_value3()};
								//since we know it will be three types
								for (int i = 0; i < 3; i++) {
									//if its the existing status then set to selected
									if (statuses[i].equals(post.getStatus())) {
										out.println("<option selected>" + statuses[i] + "</option>");
									} else {//if not, don't set to selected
										out.println("<option>" + statuses[i] + "</option>");
									}
								}
								%>
							</select>
						...
						<div class="form-label-group">
							<select class="form-control shadow" id="type" name="type">
								<%
									TypesDAO tyDAO = new TypesDAO();
								List<Types> types = tyDAO.getTypesNoPagination();
								for (Types ty : types) {
									if (ty.getName().equals(post.getType())) {
										out.println("<option selected>" + ty.getName() + "</option>");
									} else {
										out.println("<option>" + ty.getName() + "</option>");
									}
								}
								%>
							</select>
						...
						<button class="btn btn-lg btn-primary" type="submit"
							id="submit-btn">Update</button>
						<button class="btn btn-lg btn-primary" id="submit-ani" disabled>
