...
<title>Database Settings List</title>
...
<body>
	...
	if (userReq.contains("/")) {
		...
		userReq2 = userReq2.substring(userReq2.indexOf("/db-settings") + 13);
	...
	if (request.getParameter("search") == null) {
		count = ssDAO.countAllStructures();
	}

	if (request.getParameter("search") != null) {
		count = ssDAO.countSearchStructures(request.getParameter("search"));
	}
	//------------------------------------------------------------
	...
	<div style="margin-top: 50px;">
		<div class="row">
			...
			<div class="col-md-10" style="padding: 20px;" id="main">
				...
				<%-- Search --%>
				<form action="<%=um.baseUrl(request, "/backend/db-settings/1/")%>"
				...
				<div class="img-thumbnail bg-grey container"
					...
					<h4 class="text-right">
						<a href="<%=webAddress%>/backend/setting-create">Create
							Structure</a>
					</h4>
									...
									<table
										...
										<tbody>
											...
											if (request.getParameter("search") == null) {
												...
												//get data from db
												List<SiteStructure> list = ssDAO.getAllStructures(start, (int) total);
												...
												//list users in table
												for (SiteStructure ss : list) {
													out.println("<tr>" + "<td>&nbsp; &nbsp; " + No++ + "</td>" + "<td>&nbsp; &nbsp; " + ss.getId() + "</td>"
													+ "<td>&nbsp; &nbsp; " + ss.getName() + "</td>" + "<td>&nbsp; &nbsp; <a href='" + webAddress
													+ "/backend/db-setting?id=" + ss.getId() + "'>Click</a></td>" + "<td>&nbsp; &nbsp; <a href='"
													+ webAddress + "/backend/setting-edit?id=" + ss.getId() + "'>Click</a></td>" + "</tr>");
												}
											}
											...
											if (request.getParameter("search") != null) {
												...
												//get data from db
												List<SiteStructure> list = ssDAO.getSearchStructures(request.getParameter("search"), start, (int) total);
