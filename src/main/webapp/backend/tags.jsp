...
<%@ page import="dataBaseDAO.UsersDAO" import="java.util.List"
	...
	import="dataBaseModel.Tags" import="dataBaseDAO.TagsDAO"
...
<body>
	...
	if (userReq.contains("/")) {
		...
		userReq2 = userReq2.substring(userReq2.indexOf("/tags") + 6);
	...
	if (request.getParameter("search") != null) {
		count = tags.countSearchTags(request.getParameter("search"));
	}
	//------------------------------------------------------------
	...
	session.setAttribute("TagAction", "create");
	%>
	...
	<div style="margin-top: 50px;">
		<div class="row">
			...
			<div class="col-md-10" style="padding: 20px;" id="main">
				...
				<%-- Search --%>
				<form action="<%=webAddress%>/backend/tags/1/"
									...
									<form class="needs-validation" style="padding: 10px;"
										...
										<div class="form-label-group">
											<input type="text" id="inputNewTag" class="form-control"
												placeholder="Tag name" name="tagName" required> <label
												for="inputNewTag">Tag name</label>
										</div>


										<button class="btn btn-lg btn-primary" type="submit"
											id="submit-btn">Create Tag</button>
									...
									<table
										...
										<tbody>
											...
											if (request.getParameter("search") == null) {
												...
												//get data from db
												List<Tags> list = tags.getTags(start, (int) total);
												...
												//list users in table
												for (Tags tg : list) {
													String created = um.formatDbDate(tg.getCreated().substring(0, tg.getCreated().indexOf(" ")), "dd/M/yyyy");
													String updated = um.formatDbDate(tg.getUpdated().substring(0, tg.getUpdated().indexOf(" ")), "dd/M/yyyy");
													out.println("<tr>" + "<td>&nbsp; &nbsp; " + No++ + "</td>" + "<td>&nbsp; &nbsp; " + tg.getId() + "</td>"
													+ "<td>&nbsp; &nbsp; " + tg.getName() + "</td>" + "<td>&nbsp; &nbsp; " + tg.getPost_count() + "</td>"
													+ "<td>&nbsp; &nbsp; " + tg.getUsername() + "</td>" + "<td>&nbsp; &nbsp; " + created + "</td>"
													+ "<td>&nbsp; &nbsp; " + updated + "</td>" + "<td>&nbsp; &nbsp; <a href='" + webAddress
													+ "/backend/tag-edit?tagid=" + tg.getId() + "'>Click</a></td>" + "</tr>");
												}
											}
											...
											if (request.getParameter("search") != null) {
												...
												//get data from db
												List<Tags> list = tags.searchTags(request.getParameter("search"), start, (int) total);
