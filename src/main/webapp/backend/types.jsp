...
<%@ page import="dataBaseDAO.UsersDAO" import="java.util.List"
	...
	import="dataBaseModel.Types" import="dataBaseDAO.TypesDAO"
...
<body>
	...
	if (userReq.contains("/")) {
		...
		userReq2 = userReq2.substring(userReq2.indexOf("/types") + 7);
	...
	if (request.getParameter("search") != null) {
		count = types.countSearchTypes(request.getParameter("search"));
	}
	//------------------------------------------------------------
	...
	session.setAttribute("TypeAction", "create");
	%>
	...
	<div style="margin-top: 50px;">
		<div class="row">
			...
			<div class="col-md-10" style="padding: 20px;" id="main">
				...
				<%-- Search --%>
				<form action="<%=webAddress%>/backend/types/1/"
									...
									<form class="needs-validation" style="padding: 10px;"
										...
										<div class="form-label-group">
											<input type="text" id="inputNewType" class="form-control"
												placeholder="Type name" name="typeName" required> <label
												for="inputNewType">Type name</label>
										</div>


										<button class="btn btn-lg btn-primary" type="submit"
											id="submit-btn">Create Type</button>
									...
									<table
										...
										<tbody>
											...
											if (request.getParameter("search") == null) {
												...
												//get data from db
												List<Types> list = types.getTypes(start, (int) total);
												...
												//list users in table
												for (Types tp : list) {
													String created = um.formatDbDate(tp.getCreated().substring(0, tp.getCreated().indexOf(" ")), "dd/M/yyyy");
													String updated = um.formatDbDate(tp.getUpdated().substring(0, tp.getUpdated().indexOf(" ")), "dd/M/yyyy");
													out.println("<tr>" + "<td>&nbsp; &nbsp; " + No++ + "</td>" + "<td>&nbsp; &nbsp; " + tp.getId() + "</td>"
													+ "<td>&nbsp; &nbsp; " + tp.getName() + "</td>" + "<td>&nbsp; &nbsp; " + tp.getPost_count() + "</td>"
													+ "<td>&nbsp; &nbsp; " + tp.getUsername() + "</td>" + "<td>&nbsp; &nbsp; " + created + "</td>"
													+ "<td>&nbsp; &nbsp; " + updated + "</td>" + "<td>&nbsp; &nbsp; <a href='" + webAddress
													+ "/backend/type-edit?typeid=" + tp.getId() + "'>Click</a></td>" + "</tr>");
												}
											}
											...
											if (request.getParameter("search") != null) {
												...
												//get data from db
												List<Types> list = types.searchTypes(request.getParameter("search"), start, (int) total);
												//for counting item number, use the above tactive but add 1 so as not to start from 0
												int No = 1 + (Integer.parseInt(userReq) * (int) total) - (int) total;
