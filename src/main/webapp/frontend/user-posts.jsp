...
<body>
	...
	if (userReq.contains("/")) {
		...
		userReq2 = userReq2.substring(userReq2.indexOf("/my-posts") + 10);
	...
	if (request.getParameter("search") == null && request.getParameter("status") == null) {
		count = postDAO.countUserPosts(UserID, "All");
	}

	if (request.getParameter("status") != null) {
		count = postDAO.countUserPosts(UserID, request.getParameter("status"));
	}

	if (request.getParameter("search") != null) {
		count = postDAO.countSearchUserPosts(request.getParameter("search"), UserID, "All");
	}
	//------------------------------------------------------------
	...
	<div style="margin-top: 50px;">
		<div class="row">
			...
			<div class="col-md-10" style="padding: 20px;" id="main">
				...
				<%-- Search --%>
				<form action="<%=webAddress%>/my-posts/1/"
				...
				<div class="text-left img-thumbnail">
					<h4>
						...
						int pendingCount = (int) postDAO.countUserPosts(UserID, "Pending");
						if (request.getParameter("status") == null || !request.getParameter("status").equals("Pending")) {
							out.println("<a class='h6' href='" + webAddress + "/my-posts/1/?status=Pending'>Only pending (" + pendingCount
							+ ")</a>");
						}
						%>
					</h4>
									...
									<table
										...
										<tbody>
											...
											if (request.getParameter("search") == null && request.getParameter("status") == null) {
												...
												//get data from db
												List<Posts> list = postDAO.getUserPosts(UserID, "All", start, (int) total);
												...
												pendingFormat = "style='font-weight:bold;'";
													...
													+ status + "<td>&nbsp; &nbsp; <a href='" + webAddress + "/post/" + post.getLink() + "'>Click</a></td>"
													+ "<td>&nbsp; &nbsp; <a href='" + webAddress + "/edit-post?id=" + post.getId() + "'>Click</a></td>"
													+ "</tr>");
												}
											}
											...
											if (request.getParameter("status") != null) {
												...
												//get data from db
												List<Posts> list = postDAO.getUserPosts(UserID, request.getParameter("status"), start, (int) total);
												//for counting item number, use the above tactive but add 1 so as not to start from 0
												int No = 1 + (Integer.parseInt(userReq) * (int) total) - (int) total;
											...
											if (request.getParameter("search") != null) {
												...
												//get data from db
												List<Posts> list = postDAO.getSearchUserPosts(request.getParameter("search"), UserID, "All", start, (int) total);
												//for counting item number, use the above tactive but add 1 so as not to start from 0
												int No = 1 + (Integer.parseInt(userReq) * (int) total) - (int) total;
										...
										<ul class="pagination pagination-sm justify-content-center"
											...
											if (request.getRequestURI().contains(userReq)) {
												...
												reqURI = request.getRequestURI();
													...
													//if there is a /
												if (request.getRequestURI().contains("my-posts/")) {
													//just ad the number
