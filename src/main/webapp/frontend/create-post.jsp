...
<title>Orange | Create New Post</title>
			...
			<div class="col-md-10" style="padding: 20px;" id="main">
				...
				<div class="img-thumbnail bg-grey container"
					...
					<form class="needs-validation" autocomplete="off"
						action="<%=webAddress%>/PostSunmissionController" method="post"
						id="auth-form" enctype="multipart/form-data" novalidate>
						...
						<label for="profilePicture" class="custom-file-upload"
							...
							Click here to select the post image
					    ...
					    function imageCheck(){
							var test = document.getElementById("placeHolding").innerHTML.includes("Image Selected : " + document.getElementById("profilePicture").value);
							if(test){
					    		document.getElementById("placeHolding").innerHTML= "<i class='fa fa-cloud-upload' style='color:green;'></i>  Saving...";
							}else{
								document.getElementById("placeHolding").innerHTML= "<i class='fa fa-cloud-upload' style='color:red;'></i>  You must select a post image.. Click here!"; 
							}
						}
					   ...
					   $('#profilePicture').bind('change', function(){
						...
						<%
							//used to store the session values if user has entered
						//values but hasn't saved before moving to another page
						//so on reloaad he won't lose what has been typed
						String tempTitle = "";
						String tempLink = "";
						String tempExcerpt = "";
						String tempBody = "";
						...
						<%-- --------------------------------------------------------------------------- --%>
						<hr>
						<%-- tell when values have been temporarily saved --%>
						<div class="row" style="margin-left: 2px;">
							<h5>Title</h5>
							&emsp;
							<p
								style="border: 1px solid black; padding-left: 4px; padding-right: 4px; display: none;"
								id="savingAlert1">
								<span class='spinner-grow spinner-grow-sm'></span>saving...
							</p>
							<p
								style="border: 1px solid black; padding-left: 4px; padding-right: 4px; display: none;"
								id="savedAlert1">saved</p>
						</div>



						<%
							//check if any of the session exist and print value attribute to be added
						//to the HTML elements
						if (session.getAttribute("TitleTempStorage") != null) {
							tempTitle = "value='" + session.getAttribute("TitleTempStorage") + "'";
						}
						if (session.getAttribute("LinkTempStorage") != null) {
							tempLink = "value='" + session.getAttribute("LinkTempStorage") + "'";
						}
						if (session.getAttribute("ExcerptTempStorage") != null) {
							tempExcerpt = session.getAttribute("ExcerptTempStorage").toString();
						}
						if (session.getAttribute("BodyTempStorage") != null) {
							tempBody = session.getAttribute("BodyTempStorage").toString();
						}
						%>

						<div class="form-label-group">
							<input type="text" id="inputTitle" class="form-control shadow"
								placeholder="Title" <%=tempTitle%> name="title" required>
						...
						<div class="row" style="margin-left: 2px;">
							...
							&emsp;
							<p
								style="border: 1px solid black; margin-bottom: 0px; padding-left: 4px; padding-right: 4px; display: none;"
								id="savingAlert2">
								<span class='spinner-grow spinner-grow-sm'></span>saving...
							</p>
							<p
								style="border: 1px solid black; margin-bottom: 0px; padding-left: 4px; padding-right: 4px; display: none;"
								id="savedAlert2">saved</p>
						</div>
						...
						<div class="form-label-group">
							<input type="text" id="inputLink" class="form-control shadow"
								placeholder="Link" <%=tempLink%> name="link" required> <label
								for="inputLink">Link</label>
						</div>
					...
					$('#inputTitle').bind('blur', function(){
						...
						<div class="form-label-group">
							<textarea class="form-control shadow" id="inputExcerpt" rows="6"
								name="excerpt" required><%=tempExcerpt%></textarea>
							<%-- <label for="inputExcerpt">Excerpt</label> --%>
						</div>


						<script type="text/javascript">
					  <%-- transfer to temp excerpt --%>
						$('#inputExcerpt').bind('blur', function(){
							<%-- save temporarily --%>
							saveTempData();
						});
						</script>
						...
						<div class="form-label-group">
							...
							<textarea class="form-control shadow" id="body" rows="25"
								name="body" required><%=tempBody%></textarea>
						</div>
					  ...
					  <%-- transfer to temp body --%>
						...
						<%-- --------------------------------------------------------------------------- --%>
						<%-- Ajax script for above three above, for temporarily storing their values in session --%>
						<%-- this is called in their onchange event which is the script under them above  --%>
						<script type="text/javascript">					
					function saveTempData(){
					    
					    
					    var titleVal = document.getElementById('inputTitle').value;
					    var linkVal = document.getElementById('inputLink').value;
					    var excerptVal = document.getElementById('inputExcerpt').value;
					    //var bodyVal = document.getElementById('body').value;
					    //change & to its encoding value
					    var bodyVal = tinyMCE.activeEditor.getContent().replace(/&/g,'%26');
					    //change new line to its encoding value
					    bodyVal = bodyVal.replace(/\n/g,'%0D%0A');
					    
					    
					    var data =  "tempTitle=" + titleVal + "&tempLink=" + linkVal + "&tempExcerpt=" + excerptVal + "&tempBody=" + bodyVal;
					    ...
					    {
					    	if (xhr.readyState < 4){
					    		document.getElementById('savingAlert1').style.display="block";
					    		document.getElementById('savingAlert2').style.display="block";
					    		document.getElementById('savingAlert3').style.display="block";
					    		document.getElementById('savingAlert4').style.display="block";
					    	}
					    	
					        if (xhr.readyState==4 && xhr.status==200)
					        {
					        	document.getElementById('savingAlert1').style.display="none";
					        	document.getElementById('savingAlert2').style.display="none";
					        	document.getElementById('savingAlert3').style.display="none";
					        	document.getElementById('savingAlert4').style.display="none";
					        	
					        	
					        	document.getElementById('savedAlert1').style.display="block";
					        	document.getElementById('savedAlert2').style.display="block";
					        	document.getElementById('savedAlert3').style.display="block";
					        	document.getElementById('savedAlert4').style.display="block";
					        }
					    }
						
					    xhr.open("POST","PostTempSaving",true);
						...
						<div class="container" id="UIcont" style="display: none;">
							<div class="img-thumbnail row bg-grey" id="categoriesUI"></div>
						</div>
						<%-- ------------------------- --%>
					...
					function deleteElement(e){
						...
						<%-- remove the element from hidden input if its there --%>
					...
					$('#categoriesSelect').bind('change', function addElement(){
						...
						<%-- UI --%>
						<div class="container" id="UIcont2" style="display: none;">
							<div class="img-thumbnail row bg-grey" id="tagsUI"></div>
						</div>
	...
	no++;
					...
					$('#addTag').bind('click', function addElement2(){
						...
						<button type="submit" class="btn btn-primary shadow"
							id="submit-btn" onclick="imageCheck()">Submit Post</button>
